        ...
        // Combo Master pool：每局 greedy 最高連擊 ≥5，供解鎖牌背的進階玩家遭遇
        ...
        function pickCuratedSeed() {
            const hasEasyFirstPool = Array.isArray(EASY_FIRST_SEED_POOL) && EASY_FIRST_SEED_POOL.length > 0;
            const hasG2Pool = Array.isArray(G2_FLOW_SEED_POOL) && G2_FLOW_SEED_POOL.length > 0;
            const hasG3Pool = Array.isArray(G3_SOFT_CHALLENGE_SEED_POOL) && G3_SOFT_CHALLENGE_SEED_POOL.length > 0;
            if (!hasEasyFirstPool && !hasG2Pool && !hasG3Pool) return null;
            ...
            if (runIndex === 0) {
                currentGameGrade = 'g2';
                seed = pickFromPool(EASY_FIRST_SEED_POOL, 'g1');
            } else if (runIndex % 2 === 1) {
                currentGameGrade = 'g2';
                seed = pickFromPool(G2_FLOW_SEED_POOL, 'g2-' + runIndex);
            } else {
                currentGameGrade = 'g3';
                if (comboMasterActive) {
                    seed = pickFromPool(COMBO_MASTER_SEED_POOL, 'g3-combo-' + runIndex)
                        ?? pickFromPool(G3_SOFT_CHALLENGE_SEED_POOL, 'g3-' + runIndex);
                } else {
                    seed = pickFromPool(G3_SOFT_CHALLENGE_SEED_POOL, 'g3-' + runIndex);
                }
            }
